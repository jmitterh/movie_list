...
def get_iso639_1_code(language_name):
    ...
    try:
        ...
        logger.error(f"Error getting ISO-639-1 code for {language_name}: {e}")
...
def extract_language(root, file):
    ...
    logger.info(f"Extracting langauge, base file name is: {file_name}")
    ...
    if file_name.endswith('.srt'):  # Only process .srt files
        ...
        if os.path.exists(file_path):  # Check if the file exists
            logger.info(f"Attempting to extracting language from file: {file_path}")
            with open(file_path, 'r', encoding='utf-8', errors='replace') as f:
                srt_file_content = f.read()
                language = detect(srt_file_content)
                language = language.lower()
        else:
            logger.error(f"File not found: {file_path}, attempting to extract language from file name.")
            language_code = extract_language_code(file)
            logger.info(f"Language code is: {language_code}")
        ...
        if language_code is not None:
            return language_code
        logger.error(f"Could not extract language for {file}. Defaulting to eng (English).")
        return 'eng'
    # Get ISO-639-1 code
    lang = get_iso639_1_code(language)
    if lang is None and (language is not None or lang == 'und' or lang == "" or lang == " "):
        logger.error(f"Could not find ISO-639-1 code for {language}. Defaulting to eng (English).")
        return language
    logger.info(f"Language: {language}. ISO-639-1 code: {lang}")
    return lang
        ...
        if any(file.endswith(('.srt', '.idx', '.sub')) for file in os.listdir(root)) and 'Subs' not in dirs:
            ...
            for file in files:
                if file.endswith(('.srt', '.idx', '.sub')):
                    ...
                    logger.info(f"Reviewing moving root folder file: {file} from: {root}")
                    movie_folder = os.path.basename(root)
                    logger.info(f"Movie folder is: {movie_folder}")
                    movie_name = os.path.splitext(movie_folder)[0]
                    logger.info(f"Movie name is: {movie_name}")
                    ...
                    logger.info(f"New file path is: {new_file_path}")
                    ...
                    if new_file_name != file and not os.path.exists(new_file_path):
                        logger.info(f"Moving: {file} aka {new_file_name} to: {root}\n")
                    ...
                    # new file name not same as old file name and new file path with file does exist and new file name does match the regex pattern
                        ...
                        logger.info(f"Filepath already exists for {file} renamed to {new_file_name}, adding number to file name.")
                        num += 1
                        new_file_name = f"{movie_name}.{num}.{language}{sub_file_ext}"
                        new_file_path = os.path.join(root, new_file_name)
                        shutil.move(os.path.join(root, file), new_file_path)
                        logger.info(f"Updated file name is: {new_file_name} in path {new_file_path}\n")
                    # new file name same as old file name and new file path with new file name does not exist
                        ...
                        logger.info(f"File name already matches for {file}, skipping metadata update.\n")
                        continue
            ...
            for sub_root, sub_dirs, sub_files in os.walk(subs_folder):
                for sub_file in sub_files:
                    if sub_file.endswith('.srt') or sub_file.endswith('.idx') or sub_file.endswith('.sub'):
                        # extract ext from sub_file
                        logger.info(f"Reviewing Subs folder: {sub_file} from: {sub_root}")
                        sub_file_ext = os.path.splitext(sub_file)[1]
                        logger.info(f"Moving: {sub_file} to: {root}")
                        movie_folder = os.path.basename(root)
                        logger.info(f"Movie folder is: {movie_folder}")
                        movie_name = os.path.splitext(movie_folder)[0]
                        logger.info(f"Movie name is: {movie_name}")
                        ...
                        logger.info(f"Language is: {language}")
                        new_file_name = f"{movie_name}.{language}{sub_file_ext}"
                        logger.info(f"New file name is: {new_file_name}")
                        new_file_path = os.path.join(root, new_file_name)
                        logger.info(f"New file path is: {new_file_path}")
                        # new file name not same as old file name and new file path does not exist
                        if new_file_name != sub_file and not os.path.exists(new_file_path):
                            logger.info(f"Moving: {new_file_name} to: {root}\n")
                        ...
                        # new file name not same as old file name and new file path does exist and new file name does not match the regex pattern
                            ...
                            logger.info(f"Filepath already exists for {sub_file} renamed to {new_file_name}, adding number to file name.")
                            num += 1
                            new_file_name = f"{movie_name}.{num}.{language}{sub_file_ext}"
                            new_file_path = os.path.join(root, new_file_name)
                            shutil.move(os.path.join(sub_root, sub_file), new_file_path)
                        ...
                        # new file name same as old file name and new file path with new file name does not exist
                            ...
                            logger.info(f"File name already matches for {sub_file}, But is in sub folder, moving to root folder.\n")
                            shutil.move(os.path.join(sub_root, sub_file), new_file_path)
                        # new file name same as old file name with regex, still need to move to root folder
                        elif new_file_name == re.sub(r'\.\d+\.', '.', sub_file):
                            logger.info(f"Rare accurance, New file name {new_file_name} matches with old {sub_file}. Adding number to file name.")
                            num += 1
                            new_file_name = f"{movie_name}.{num}.{language}{sub_file_ext}"
                            new_file_path = os.path.join(root, new_file_name)
                            shutil.move(os.path.join(sub_root, sub_file), new_file_path)
                            ...
                            logger.info(f"File name: {new_file_name} already matches for {sub_file}, skipping metadata update.\n")
                            continue
                
                shutil.rmtree(subs_folder)
        
        else:
            logger.info(f"No SRT files found or no SRT files to edit, for {root}.")
            movie_folder = os.path.basename(root)
            movie_name = os.path.splitext(movie_folder)[0]
            movies_without_srt_files = movie_name
            logger.info(f"Movie name is: {movie_name}, appending to list of movies without SRT files.\n")
...
if __name__ == "__main__":
    ...
    df = pd.DataFrame()
    logger.info(f"Starting program for folder: {local_path}")
